...
Route::group(['prefix' => 'api'], function() {
    ...
    Route::get('webservices/notifications', function (\Request $request) {
        ...
        if($notifications){
            ...
            if(isset($theme->getConfigArray('translate')[$currentLang])){
    ...
    Route::any('uploadfiles/{id}', function(Request $req,$id) {
        $file = new \System\Models\File;
        $file->data = \Input::file('files');
        $file->save();
        $order  =   \RainLab\User\Models\User::find($id);
        //$order->id_copy()->add($file);
    });
        ...
        foreach($records as $record){
            ...
            foreach($languages as $lang){
                if(isset($record->message_data[$lang->code])){
                    $add = json_encode($record->message_data[$lang->code]);
                    $recordArray[$lang->name] =   '<label rel="'.$record->id.'" data-language="'.$lang->code.'" class="translate">'.$add.'</label>';
                }elseif($lang->code == 'en'){
                    $add = json_encode($record->message_data['x']);

                    if(!is_scalar($record->message_data['x']) && array_key_exists('date', $record->message_data['x'])) {
                        $recordArray[$lang->name] = '';
                    } else {
    ...
    Route::any('selectcountries', function(Request $req) {
        $data = array();
        if(Request::get('id') !== null){
            $countries  =   \RainLab\Location\Models\Country::select('id','name')->where('is_enabled', 1)->where('id',Request::get('id'))->get();
            foreach($countries as $country){
                $data[] = array("value"=>$country['id'], "text"=>$country['name']);
            }
        }
        if(Request::get('q') !== null){
            $countries  =   \RainLab\Location\Models\Country::select('id','name')->where('is_enabled', 1);
    ...
    Route::any('selectstates', function(Request $req) {
        $data = array();
        if(Request::get('id') !== null){
            $countries  =   \RainLab\Location\Models\State::select('id','name')->where('id',Request::get('id'))->get();
            foreach($countries as $country){
                $data[] = array("value"=>$country['id'], "text"=>$country['name']);
            }
        }
        if(Request::get('q') !== null){
            $countries  =   \RainLab\Location\Models\State::select('id','name')->where('country_id', Request::get('dependency_id'));
            $countries  =   $countries->where('name', 'like','%'.Request::get('q').'%');
    ...
    Route::any('selectcities', function(Request $req) {
        $data = array();
        if(Request::get('id') !== null){
            $countries  =   \Spot\Shipment\Models\City::select('id','name')->where('id',Request::get('id'))->get();
            foreach($countries as $country){
                $data[] = array("value"=>$country['id'], "text"=>$country['name']);
            }
        }
        if(Request::get('q') !== null){
            $countries  =   \Spot\Shipment\Models\City::select('id','name')->where('state_id', Request::get('dependency_id'));
            $countries  =   $countries->where('name', 'like','%'.Request::get('q').'%');
            $count      =   $countries->count();
            $countries  =   $countries->get();
            foreach($countries as $country){
    ...
    Route::any('selectareas', function(Request $req) {
        $data = array();
        if(Request::get('id') !== null){
            $countries  =   \Spot\Shipment\Models\Area::select('id','name')->where('id',Request::get('id'))->get();
            foreach($countries as $country){
                $data[] = array("value"=>$country['id'], "text"=>$country['name']);
            }
        }
        if(Request::get('q') !== null){
            $countries  =   \Spot\Shipment\Models\Area::select('id','name')->where('city_id', Request::get('dependency_id'));
            $countries  =   $countries->where('name', 'like','%'.Request::get('q').'%');
            $count      =   $countries->count();
            $countries  =   $countries->get();
            foreach($countries as $country){
    ...
    Route::any('clienttransactions', function(Request $req) {
        ...
        $records    =   \Spot\Shipment\Models\Transaction::where('user_id', $request['id'])->orderBy($field, $sort);
        if(isset($request['query'])){
            if(isset($request['query']['generalSearch']) && $request['query']['generalSearch'] != ''){
                $search     =   $request['query']['generalSearch'];
                $records    =   $records->where(function($q) use($search){
    ...
    Route::any('transactions', function(Request $req) {
        ...
        $records    =   \Spot\Shipment\Models\Transaction::orderBy($field, $sort);
    ...
    Route::any('containers/{type}', function(Request $req,$type) {
        ...
        foreach($records as $record){
            ...
            $recordArray = array(
                ...
                'status'                =>  $record->status,
                'destination'           =>  ($record->destination ? $record->thecountry :null),
    ...
    Route::any('shipments/{type}', function(Request $req,$type) {
        ...
        if($type == 'manifest'){
            ...
            if($type == 'requests'){
                ...
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[4,5,12])->where(function($q){$q->where('assigned_id', '!=', null)->orWhere('manifest_id', '!=', null);})->orderBy($field, $sort);
            }elseif($type == 'withdriver'){
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[5,12])->orderBy($field, $sort);
            }elseif($type == 'processing'){
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[3,6,7,8,10,11])->orderBy($field, $sort);
            }elseif($type == 'delayed'){
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[0,1,3,4])->where('delivery_date', '<', \Carbon\Carbon::now()->format('Y-m-d'))->orderBy($field, $sort);
            }elseif($type == 'saved'){
                ...
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[3,14])->orderBy($field, $sort);
                ...
                $records    =   \Spot\Shipment\Models\Order::whereIn('requested',[6,10,11,8])->orderBy($field, $sort);
        ...
        foreach($records as $record){
            ...
            if($type == 'manifest'){
                ...
                $recordArray = array(
                    ...
                    'sender_city'           =>  (($record->sender_city)?$record->senderCity : '-'),
                    'receiver'              =>  (($record->receiver) ? $record->receiver : '-'),
                    'receiver_name'         =>  (($record->receiver_name) ? $record->receiver_name : '-'),
                    'receiver_city'         =>  (($record->receiver_city) ? $record->receiverCity : '-'),
                    'status'                =>  $record->status,
                    'requested'             =>  $record->requested,
                    'delayed'               =>  $delayed,
                    'manifest_id'           =>  $record->manifest_id,
    ...
    Route::any('get/bid' , function (Request $req){
        $request = post();
