<?php 
use \Spot\Shipment\Models\Settings;use RainLab\Translate\Classes\Translator;use RainLab\Translate\Components\LocalePicker;class Cms609bc9207e6d5386911950_b895fd2e388b308a5038affda27e7aa7Class extends Cms\Classes\LayoutCode
{
...
public function onStart()
{
    ...
    $this['getList'] = new class {
        ...
        {
            $item         =   \RainLab\Location\Models\Country::where('id', $selected)->first();
            if($item)
            echo $item->name;
        }
        public function state($selected)
        {
            $item         =   \RainLab\Location\Models\State::where('id', $selected)->first();
            if($item)
            echo $item->name;
        }
        public function city($selected)
        {
            $item         =   \Spot\Shipment\Models\City::where('id', $selected)->first();
            if($item)
            echo $item->name;
        }
        public function area($selected)
        {
            $item         =   \Spot\Shipment\Models\Area::where('id', $selected)->first();
            if($item)
            echo $item->name;
        }
    };
